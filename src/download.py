...
def download_single_file(
    ...
    fn = custom_fn or (
       '_'.join(variable + pressure_level + year + [str(resolution[0])]) + '.nc'
        #f'{variable}_{pressure_level}_{year}_{str(resolution[0])}.nc'  #'_'.join(variable + pressure_level + year) + '.nc'
    )
...
def main(
    ...
    :param day: Day(s) to download data
    :param time: Hour(s) to download data. Format: 'hh:mm'
    :param resolution: Resolution of grid. Format: [lon, lat]
    :param custom_fn: If not None, use custom file name. Otherwise infer from parameters.
    """
